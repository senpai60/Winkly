...
export function SwipeScreen({ onProfileDetails, onNFTDate, onMyProfile, onDashboard }) {
  ...
  if (isLoading) {
    return <div className="min-h-screen bg-background flex items-center justify-center"><p className="text-white">Finding people for you...</p></div>;
  }

  if (error) {
    return <div className="min-h-screen bg-background flex items-center justify-center"><p className="text-red-500">{error}</p></div>;
  }
  ...
  return (
    <div className="min-h-screen bg-background p-4 flex flex-col">
      ...
      >
        <Button onClick={onMyProfile} variant="ghost" size="sm" className="hover:bg-white/10 p-2">
          <User className="w-6 h-6" />
        </Button>
        <h1 className="gradient-text-pink-blue font-bold text-2xl">Discover</h1>
        <Button onClick={onDashboard} variant="ghost" size="sm" className="hover:bg-white/10 p-1">
                ...
                <div className="relative h-full">
                    <ImageWithFallback
                        src={currentProfile.images && currentProfile.images.length > 0
                            ? `http://localhost:5000/${currentProfile.images[0].replace(/\\/g, '/')}`
      ...
      >
        <Button onClick={() => handleSwipe('left')} size="lg" variant="outline" className="w-14 h-14 rounded-full border-red-500/30 hover:border-red-500 hover:bg-red-500/10 hover:glow-pink">
          <X className="w-6 h-6 text-red-500" />
        </Button>
        <Button onClick={() => handleSwipe('right')} size="lg" className="w-14 h-14 rounded-full bg-primary hover:bg-primary/80 glow-pink">
          <Heart className="w-6 h-6" />
        </Button>
        <Button onClick={() => onNFTDate(currentProfile)} size="lg" variant="outline" className="w-14 h-14 rounded-full border-accent/30 hover:border-accent hover:bg-accent/10 hover:glow-blue">
